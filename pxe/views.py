    ...
    if bios_mode_sub == "legacy":
        ...
        if os_sub_version_max_sub == "7":
            string_to_write.append(
                "kernel http://{ipaddress_dhcp}/images-uefi/{os_version}/{os_version}{os_sub_version_max}-{os_sub_version_min}_{os_bit}/vmlinuz initrd=initrd.img initrd=initrd.img inst.dd=http://{ipaddress_dhcp}/driver/megaraid_sas-07.705.02.00_el{os_sub_version_max}.{os_sub_version_min}-1.x86_64.iso/dd.iso inst.dd=http://{ipaddress_dhcp}/driver/mpt3sas-24.00.00.00_el{os_sub_version_max}.{os_sub_version_min}-1.x86_64.iso/dd.iso modprobe.blacklist=qat_c62x ip=dhcp inst.ks=http://{ipaddress_dhcp}/ks/ks_all/{mac_net_pxe}.cfg".format(
            ...
            string_to_write.append(
                "kernel http://{ipaddress_dhcp}/images-uefi/{os_version}/{os_version}{os_sub_version_max}-{os_sub_version_min}_{os_bit}/vmlinuz initrd=initrd.img ramdisk_size=8192 dd=http://{ipaddress_dhcp}/driver/megaraid_sas-07.705.02.00_el{os_sub_version_max}.{os_sub_version_min}-1.x86_64.iso/dd.iso dd=http://{ipaddress_dhcp}/driver/mpt3sas-24.00.00.00_el{os_sub_version_max}.{os_sub_version_min}-1.x86_64.iso/dd.iso ip=dhcp ks=http://{ipaddress_dhcp}/ks/ks_all/{mac_net_pxe}.cfg ksdevice={mac_boot_device_rhel6}".format(
                    os_version=os_version_sub, os_sub_version_max=os_sub_version_max_sub,
                    os_sub_version_min=os_sub_version_min_sub, os_bit=os_bit_sub, ipaddress_dhcp=ipaddress_dhcp_sub,
        ...
        if os_sub_version_max_sub == "7":
            string_to_write.append(
                "linux (http)/images-uefi/{os_version}/{os_version}{os_sub_version_max}-{os_sub_version_min}_{os_bit}/vmlinuz inst.dd=http://{ipaddress_dhcp}/driver/megaraid_sas-07.705.02.00_el{os_sub_version_max}.{os_sub_version_min}-1.x86_64.iso/dd.iso inst.dd=http://{ipaddress_dhcp}/driver/mpt3sas-24.00.00.00_el{os_sub_version_max}.{os_sub_version_min}-1.x86_64.iso/dd.iso  modprobe.blacklist=qat_c62x ip=dhcp inst.ks=http://{ipaddress_dhcp}/ks/ks_all/{mac_net_pxe}.cfg".format(
                    os_version=os_version_sub, os_sub_version_max=os_sub_version_max_sub,
                    os_sub_version_min=os_sub_version_min_sub, os_bit=os_bit_sub, ipaddress_dhcp=ipaddress_dhcp_sub,
                    mac_net_pxe=mac_net_pxe_sub) + os.linesep)
        elif os_sub_version_max_sub == "6":
            string_to_write.append(
                "linux (http)/images-uefi/{os_version}/{os_version}{os_sub_version_max}-{os_sub_version_min}_{os_bit}/vmlinuz ip=dhcp dd=http://{ipaddress_dhcp}/driver/megaraid_sas-07.705.02.00_el{os_sub_version_max}.{os_sub_version_min}-1.x86_64.iso/dd.iso dd=http://{ipaddress_dhcp}/driver/mpt3sas-24.00.00.00_el{os_sub_version_max}.{os_sub_version_min}-1.x86_64.iso/dd.iso ks=http://{ipaddress_dhcp}/ks/ks_all/{mac_net_pxe}.cfg ksdevice={mac_boot_device_rhel6}".format(
                    os_version=os_version_sub, os_sub_version_max=os_sub_version_max_sub,
                    os_sub_version_min=os_sub_version_min_sub, os_bit=os_bit_sub, ipaddress_dhcp=ipaddress_dhcp_sub,
                    mac_net_pxe=mac_net_pxe_sub, mac_boot_device_rhel6=mac_boot_device_rhel6_sub) + os.linesep)
        string_to_write.append(
...
def generate_ks_redhat_centos(os_version, os_sub_version, mac_net_pxe_temp, mac_boot_device_rhel6, bios_mode, os_bit, os_disk):
    ...
    try:
        ...
        message_info = "无法连接至DHCP服务器1，请检查网络连接！"
        flag_status = 0
        return flag_status, message_info
    ...
    try:
        ...
        try:
            ...
            message_info = "无法连接至DHCP服务器2，请检查网络连接！"
            sftp_upload_ks.close()
            upload_menu.close()
            flag_status = 0
            return flag_status, message_info
        ...
        message_info = "无法连接至DHCP服务器3，请检查网络连接！"
        flag_status = 0
        return flag_status, message_info
...
def generate_ks_ubuntu(os_version, os_sub_version, mac_net_pxe_temp, mac_boot_device_rhel6, bios_mode, os_bit, os_disk):
    ...
    for item_data_pre in handler_ks_change:
        if re.search(pattern_os_disk, item_data_pre.decode()):
            data_pre = re.sub(pattern_os_disk, "/dev/%s" % os_disk, item_data_pre.decode())
            string_pre += data_pre
        else:
            string_pre += item_data_pre.decode()
    handler_ks_change.close()
    ...
    if request.method == "POST":
        if "gen_os_ver" in request.POST:
            ...
            if os_type == 'redhat':
                ...
                os_version_list = ['14.04.5', '16.10', '17.04', '17.10', '16.04', '15.10']
